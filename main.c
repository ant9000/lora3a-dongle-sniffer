...
void dump_message(const char *message, size_t len, uint8_t *rssi, int8_t *snr)
{
    uint16_t cnt, net;
    uint8_t src, dst;
    ...
    if(len>8 && uint16(message) == 0x00e0) {
        ptr += 2; n -= 2;
        cnt = uint16(ptr);
        ptr += 2; n -= 2;
        net = uint16(ptr);
        ...
        src = (uint8_t)ptr[0];
        ptr++; n--;
        printf("EMB packet: counter=%04x, network=%04x, dst=%02x, src=%02x. Payload:\n", cnt, net, dst, src);
    } else {
        printf("RAW packet:\n");
    }
